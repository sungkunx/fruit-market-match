...
| 헛밀기 | 원래 자리로 되돌리고 배수·콤보 초기화 (판은 섞지 않음) |
...
- `result.valid`가 거짓이면:
  - 점수 0
  - 새 상태 `{ multiplier: 1.0, comboCount: 0, lastMatchedFruit: null }`
- 매칭이 있으면:
...
**랭킹 등록 기준:** 기존 5000점 기준을 `js_file.js` 상단 상수 `LEADERBOARD_MIN_SCORE = 5000`으로 뺀다. 값은 플레이 테스트 후 조정한다.
